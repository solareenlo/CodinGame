...
inline void mapTranslation(VVI &from, VVI &to, P ori) {
    int height = from.size();
    int width = from[0].size();
    int diffX =  ori.first - (width / 2);
    if (diffX <= 0) {
...
inline int calDist(P from, P to, VVI map) {
    ...
    VVI field(height, VI(width, 0));
    // cerr << "map: " << height << ' ' << width << endl;
    // cerr << "ori: " << from.second << ' ' << from.first << endl;
    // REP(i, height) {
    //     REP(j, width) cerr << setw(2) << map[i][j] << ' ';
    //     cerr << endl;
    // }
    ...
    REP(i, height)
        if (map[i][0] == 1)
            loop = true;
    cerr << "mapTranslation: " << loop << endl;
    if (loop) {
        mapTranslation(map, field, from);
        from.first -= diffX;
    } else {
        REP(i, height) REP(j, width)
            field.at(i).at(j) = map.at(i).at(j);
    }

    // REP(i, height) {
    //     REP(j, width) cerr << setw(2) << field[i][j] << ' ';
    //     cerr << endl;
    // }
    cerr << "ori: " << from.second << ' ' << from.first << endl;
    ...
    REP(i, height) {
        REP(j, width) cerr << setw(2) << dist[i][j] << ' ';
        cerr << endl;
    }

    cerr << width << ' ' << height << endl;
    VVI tmp(height, VI(width, -1));
    P inverse = make_pair(width - 1 - (from.first + diffX), from.second);
    mapTranslation(dist, tmp, inverse);
    REP(i, height) {
        REP(j, width) cerr << setw(2) << tmp[i][j] << ' ';
        cerr << endl;
    }
    return 0;
}
...
int main() {
    ...
    while (1) {
        ...
        // }
        int tmp = calDist(myPac[0], pellet.begin()->first, map);
